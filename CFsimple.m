...
num_catagory = 108;
num_feature=108;
p1=1.8;%purchasing parameter
p2=0.5;%clicking parameter
p3=0.8;%recycling parameter
p4=0.2;%basic adjust parameter for recycle data
p5=1.0;%maxlimit adjust value
lambda = 5;
N=10;
ite=100;
train_rate=0.8;
test_rate=0.2;
Rec = load('recycle_cleared.txt');%user/catagory
Buy = load('buy_clear.txt');%user/item/catagory
Cli = load('click_cleared.txt');%user/item/catagory
...
for i=1:floor(train_rate*ss)
    j=index(i);
    Y_train(Cli(j,1),Cli(j,2))= Y_train(Cli(j,1),Cli(j,2))+p2;
...
for i=ceil(train_rate*ss):ss
    j=index(i);
    Y_test(Cli(j,1),Cli(j,2))= Y_test(Cli(j,1),Cli(j,2))+p2;
...
for i=1:size(Rec,1)
    ...
    for j=1:num_item
        if Y_train(us,j)>0 && shop(j)==cl
            TY_train(us,j)=TY_train(us,j)+adjust;
            if TY_train(us,j)>5
                TY_train(us,j)=5;
            end         
        end
        if Y_test(us,j)>0 && shop(j)==cl
            TY_test(us,j)=TY_test(us,j)+adjust;
            if TY_test(us,j)>5
                TY_test(us,j)=5;
            end
        end
